# Adapted from https://github.com/md4all/md4all/blob/main/losses/SupervisedLoss.py
...
class SupervisedLoss(nn.Module):
    ...
    def forward(self, inputs, outputs):
        ...
        do_flip = outputs['is_flipped']
        cond_flip = lambda x: flip_lr(x) if do_flip else x
        with torch.no_grad():
            #Feature alignment needs to be consistent
            gt_inv_depth,teacher_feature = self.teacher_net(cond_flip(inputs[("color", 0)]), inputs['weather'])
            gt_inv_depths = [cond_flip(gt_inv_depth[("disp", 0, scale)]) for scale in self.scales]
        depth_S, depth_T = 1 / inv_depths[0].detach(), 1 / gt_inv_depths[0]
        mask_S = torch.abs(depth_S - depth_T).detach()
        mask_S_m = mask_S / (depth_S + depth_T)
        outputs['Mask'] = mask_S_m
        # Calculate supervised loss
        loss = self.calculate_loss(inv_depths, gt_inv_depths)
        # Return losses and metrics
        return loss,teacher_feature
