# Adapted from https://github.com/md4all/md4all/blob/main/config/config.py
# Most of the parameters are the same as md4all
import argparse
...
# General settings
_C.SYSTEM = CN()
_C.SYSTEM.STRATEGY = 'auto' # 'ddp_find_unused_parameters_true'
_C.SYSTEM.ISDIST = False  # Distributed          
...
_C.SYSTEM.NUM_WORKERS = 16  # Number of workers for the dataloader
...
_C.TRAINING = CN()
_C.TRAINING.EPOCHS = 25  # Number of epochs to train
...
_C.TRAINING.ACCUMULATE_GRAD_BATCHES = 1  # Accumulates gradients over this number of batches before stepping the optimizer (Todo: Must be changed to 1 for newer pl versions)

_C.VALIDATION = CN()
_C.VALIDATION.BATCH_SIZE = 16  # Batch size for validation

_C.SAVE = CN()
_C.SAVE.CHECKPOINT_PATH = 'log/checkpoints'  # Folder within ACDepth to store checkpoints
...
_C.SAVE.MONITOR = 'metrics_night/everything/a1_pp'  # Metric to monitor
_C.SAVE.MODE = 'max'  # Mode for the metric to monitor
_C.SAVE.TOP_K = 3  # Number of top models (w.r.t. monitoring metric) to save
_C.SAVE.LAST = True  # For saving last checkpoint

_C.LOAD = CN()
_C.LOAD.CHECKPOINT_PATH = None
_C.LOAD.DV2_PATH = './checkpoints/depth_anything_v2_vits.pth'  # Path to the Depthanythingv2 model weights
_C.LOAD.DV2_ENCODER = 'vits'  # Encoder type for DepthAnythingV2
...
_C.DATASET = CN()
_C.DATASET.DATAROOT = './datasets/nuscenes/'#'/data_ssd/jing/Datasets/nuscenes/'  # Path to main data folder containing nuScenes or RobotCar
...
_C.DATASET.TEMP_CONTEXT = [0]  # Temporal context frame to use (was never changed)

_C.DATASET.LOAD = CN()
_C.DATASET.LOAD.GT = CN()
_C.DATASET.LOAD.GT.DEPTH = True  # Load ground truth depth in dataloader
_C.DATASET.LOAD.GT.POSE = False  # Load ground truth pose in dataloader
...
_C.DATASET.AUGMENTATION.COLOR_JITTER.SAMPLE_KEYS = ['color_aug']  # Apply color jitter to
...
_C.DATASET.AUGMENTATION.GAUSSIAN_NOISE.SAMPLE_KEYS = ['color_aug']  # Apply gaussian noise to
...
_C.DATASET.AUGMENTATION.GAUSSIAN_BLUR.SAMPLE_KEYS = ['color_aug']  # Apply gaussian blurring to

_C.DATASET.AUGMENTATION.NORMALIZE = CN()
_C.DATASET.AUGMENTATION.NORMALIZE.MODE = 'Daytime'  # Normalization type (Dataset, Daytime, Image) => Use Image mode if daytime is not known a priori instead of Dataset
...
_C.LOSS.RANK_WEIGHT= 0.0
_C.LOSS.CONTRAST_WEIGHT= 0.0
_C.LOSS.CONTRAST_START = 15
_C.LOSS.CONTRAST_LOSS_W1 = 1.0  # Weight for contrast loss 
_C.LOSS.CONTRAST_LOSS_W2 = 0.5  # Weight for contrast loss 
_C.LOSS.RANKING_WINDOWS_GT = False  # Ranking window for ground truth
_C.LOSS.RANKING_WINDOWS_PRE = True  # Ranking window for predictions
_C.LOSS.SAMPLE_RATIO = 0.05  # Sample ratio for ranking loss
_C.LOSS.LOCAL_SAMPLE_RATIO = 0.05  # Number of ranking windows

_C.OPTIMIZER = CN()
_C.OPTIMIZER.DEPTH_LR = 5e-4  # Learning rate for depth network
_C.OPTIMIZER.POSE_LR = 2e-4  # Learning rate for pose network
_C.OPTIMIZER.WEIGHT_DECAY = 0.0  # Weight decay for optimizer
_C.OPTIMIZER.STEP_SIZE = 15  # Step size for optimizer
_C.OPTIMIZER.GAMMA = 0.1 # Gamma for optimizer
...
parser = argparse.ArgumentParser(description='ACDepth training')
def get_parser():
