...
class Evaluator:
    ...
    def evaluate_depth(self, batch):
        ...
        # Get predicted depth
        outputs,_ = self.model(batch["color", 0], batch["weather"])
        depth = inv2depth(outputs[("disp", 0, 0)])
        # Post-process predicted depth
        batch[("color", 0)] = flip_lr(batch[("color", 0)])
        inv_depths_flipped,_ = self.model(batch["color", 0], batch["weather"])
        inv_depths_flipped = inv_depths_flipped[("disp", 0, 0)]
