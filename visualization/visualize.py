...
class Visualizer:
    ...
    def visualize_depth_gt(self, img_rgb, img_lidar, filename):
        ...
        plt.scatter(indices_x, indices_y, s=1.5, c=z, edgecolors='none', cmap="viridis", vmin=1.0, vmax=self.max_depth)
    ...
    def visualize_test(self, batch, outputs):
        ...
        imgs = batch[('color_original', 0)].cpu() if ('color_original', 0) in batch else interpolate_image(batch[('color', 0)], [height, width]).cpu()
