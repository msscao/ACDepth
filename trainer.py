# Adapted from https://github.com/md4all/md4all/blob/main/trainer.py
...
class ACDepth(pl.LightningModule):

    def __init__(self, cfg, is_train):
        super(ACDepth, self).__init__()
        ...
        self.contrast_flag = cfg.LOSS.CONTRAST_WEIGHT > 0
        self.contrast_start = cfg.LOSS.CONTRAST_START
        self.depth_model = DepthNet(cfg)
        self.pose_model = PoseNet(cfg) if cfg.LOSS.VELOCITY_WEIGHT > 0 else None 
    ...
    def depth_estimation(self, inputs, is_train):
        ...
        outputs, feature_aug = self.depth_model(cond_flip(inputs[('color_aug', 0)]), inputs['weather_depth'])
        inv_depth = {scale: cond_flip(disp) for scale, disp in outputs.items()}
        if inputs['epoch'] > self.contrast_start  and self.contrast_flag:
            with torch.no_grad():
                _,feature = self.depth_model(cond_flip(inputs[('color', 0)]), inputs['weather'])
            inv_depth['feature'] = feature

        inv_depth['feature_aug'] = feature_aug
        inv_depth['is_flipped'] = do_flip
        return inv_depth
    ...
    def shared_step(self, inputs, is_train):
        inputs['epoch'] = self.trainer.current_epoch
        outputs = self.depth_estimation(inputs, is_train)
        if self.pose_model:
        ...
        for condition in metrics.keys():
            for obj_class in metrics[condition].keys():
                for metric_name, metric_value in metrics[condition][obj_class].items():
                    self.log(f"metrics_{condition}/{obj_class}/{metric_name}", metric_value,sync_dist=True)
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        outputs,_ = self.depth_model(batch[('color', 0)], batch.get('weather'))
